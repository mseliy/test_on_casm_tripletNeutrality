...
public:
   ...
   void assort_a2(int i, int j);           // assort data according to the ith column, then jth column, in ascending order, i, j is from 1
   void assort_d2(int i, int j);           // assort data according to the ith column, then jth column, in descending order, i, j is from 1
   ...
   Array half_hull3(Array &f);              //find hull points and facet on the bottom half hull, for 3D
   ...
   double elem(int i, int j) {return ptr[(i-1)*nc+j-1];}	// get the matrix element (i,j), note that i and j 
   ...
   int num_row() {return nl;}                 // return number of rows
   int num_col() {return nc;}				  // return number of columns							   
...
int Array::getSize() const { cout<<"the dimension is "<<nl<<"x"<<nc<<endl;return nl*nc; }
...
double Array::det()
{
 ...
 if(nl==1){deter=ptr[0];}
 else if(nl==2){deter = ptr[0]*ptr[3]-ptr[2]*ptr[1];}
 else{ deter=0;
       for(j1=0;j1<nl;j1++){
           for(i=1;i<nl;i++){
               j2=0;
...
Array Array::inverse()
{
   ...
   double ib;
   if (det()==0) {cout<<"singular!"<<endl; return inv;}
   if(nl==1) {inv.ptr[0]=1.0/ptr[0]; return inv;}
   else{
...
Array Array::LU()
{
   ...
   double  sum,dum;
	...
	for (j=0; j<nl; j++){
      ...
      if (alu.ptr[j*nc+j]==0.0) {alu.ptr[j*nc+j]=TINY; cout<<"diagonal element is zero"<<endl;}
		if (j!=nl){
...
Array Array::solveleq()
{
  ...
  for( i=0; i<nl; i++){
   ...
   for(k=0;k<=nl-2;k++)
   {
      ...
      { if(js[k]!=k)
      	for(i=0;i<=nl-1;i++)
         { p=i*nl+k;q=i*nl+js[k];
           t=a.ptr[p];a.ptr[p]=a.ptr[q];a.ptr[q]=t;
         }
         if(is!=k)
         {for(j=k;j<=nl-1;j++)
           { p=k*nl+j;q=is*nl+j;
          ...
          t=b.ptr[k];b.ptr[k]=b.ptr[is];b.ptr[is]=t;
         }
      }
      if(l==0)
      ...
      for(k=nl-1;k>=0;k--)
      	if(js[k]!=k)
         {t=solu.ptr[k];solu.ptr[k]=solu.ptr[js[k]];solu.ptr[js[k]]=t;}
         delete[] js;
         return solu;
   }
	...
	for (k=nc;k>=1;k--){
	     for (its=1; its<=30;its++){
			  ...
			  if (flag) {
				...
				if (its==30) {cout<<"no convergence in 30 svd iterations"; exit(1);}
				x=w[l];
				nm=k-1;
				y=w[nm];
				g=rv1[nm];
...
Array Array::svdls(Array &Y, Array &covar, double &rms, double &cv, int &cvn)
{
   ...
   for(i=1; i<=nl; i++){
	   ...
	   if ((a-1.0<1.0e-11)&&(a-1.0>-1.0e-11)) {cvn--;continue;} // if a=1.0, then actual Ei=fitted Ei, don't include this
   ...
   // cout<<"Divide time "<<b_num<<endl;;
    ...
    for(fit_num=0; fit_num<Nt; fit_num++){ //randomly select Nt data points to form fitting subset
       do{
	     ...
	     for(i=0; i<Nt; i++) 
		    if(fit_array[i]==fit_index) {exist=true; break;}
	   }
	   while(exist==true);		
    ...
    for(i=1; i<=nl; i++){
	   in_fit=false;
	   for(j=0; j<Nt; j++) if(i==fit_array[j]) {in_fit=true; break;} 
	   if(in_fit==false) {non_fit_array[k]=i; k++;}
	}   
...
double Array::cv(Array &Y, int &num_cv)  // Y stores the actual energies
{
	...
	for(ii=0; ii<nl; ii++){
	   ...
	   for(m=1;m<=nc;m++)
          if(W_no_i.elem(m,1)<1.0e-8) {linear=true;continue;}
	   if(linear) continue;
...
Array Array::inverSVD()
{
  ...
  thresh=nl*TOL*wmax;
  for (int j=0;j<nc;j++){
	   if (w.ptr[j] < thresh) {cout<<"This matrix is singular"; exit (1) ;}
...
{
  Array row(1,nc);
  if(i>nl) {cout<<"Error! The number of row is beyond the row range!"; exit (1);}
  for(int j=0; j<nc; j++){
...
{
  Array column(nl,1);
  if(j>nc) {cout<<"Error! The number of column is beyond the column range!"; exit (1);}
  for (int i=0; i<nl; i++){
 ...
 for(m=0; m<nl-1; m++){
     ...
     for(i=m+1; i<nl; i++){
         ...
         for(j=0; j<nl; j++){
            ...
            if(areaA.det()<0) {h=false;break;} // if for all other points except A and B, the area is positive or zero
...
///////////////////////////////////////////////////////////////////////////
void Array:: assort_a2(int ci, int cj)  //assort data according to the ci column and then cj column in ascending order, ci, cj is from 1
{            //  Begin assort_a2
 int i, j, k, m, screen;
 int Num1 = 1;
 int Num2 = 1;
 assort_a(ci);
 queue <int> second_count;
 for (i=0; i<nl-1; i++){   //LOOP 1
	if (ptr[i*nc+ci-1]!=ptr[(i+1)*nc+ci-1]) {
	  second_count.push(Num2); Num2 = 1; 
	  if(i==nl-2) second_count.push(Num2);
	  Num1++;
	}
	else{
	  Num2++;
 ...
 queue <Array> sorter;
 //cout<<nl<<"  "<<nc<<endl;
 for(i=0; i<Num1; i++){   //LOOP 2
	double *temp = new double [second_count.front()*nc];
	//cout<<i<<" Num2 is "<<second_count.front()<<endl;
	//cin>>screen;
    for(j=cum; j<cum+second_count.front(); j++)
	   for(k=0; k<nc; k++) 
	      temp[(j-cum)*nc+k]=ptr[j*nc+k];
	Array tempA(second_count.front(), nc);
	tempA.setArray(temp);
	//cout<<tempA<<endl;
	//cin>>screen;
	tempA.assort_a(cj);
	for(m=0; m<tempA.nl; m++) sorter.push(tempA.row(m+1));
	cum += second_count.front();
	second_count.pop();
	delete [] temp;
 }                        //LOOP 2
 Array temp2A(1, nc);
 for(i=0; i<nl; i++){
    temp2A = sorter.front();
	for(j=0; j<nc; j++) ptr[i*nc+j]=temp2A.ptr[0*nc+j];
	sorter.pop();
 }
}  //  end assort_a2
///////////////////////////////////
void Array:: assort_d2(int ci, int cj)  //assort data according to the ci column and then cj column in descending order, ci, cj is from 1
{            //  Begin assort_d2
 int i, j, k, m, screen;
 int Num1 = 1;
 int Num2 = 1;
  ...
  if (n==2) {
     ...
     for(i=1; i<nl; i++){
         ...
         for(j=0; j<nl; j++){
            ...
            else if(area_value<0) {h=false;break;} // if for all other points except A and B, the area is positive or zero
            else continue;
	      }                                     // that means all points C is to the left of line AB, then keep B as a
		                                       // hull point
	      if (h==true) {
  ...
  return r_base;
      ...
      for(i=n-1; i<nl; i++){
         ...
         for(j=0; j<nl; j++){
            ...
            if (count==1){pre_sign=sign; continue;}
            if(sign!=pre_sign){h=false;break;}
            pre_sign=sign;
	      }
	      if (h==true) {
 ...
 for(i=0; i<n; i++)
     for(j=0; j<n; j++){
         if(j==i) continue;
                   ...
                   tran[r]=ptr[j*nc+r];
               tranA.setArray(tran);
               new_elem.push(tranA);}  // push new sub_element from hull base to queue new_elem
     }
...
{ 
  ...
  if (n==2) {// if #1
      ...
      Array area(3,3);
      area.ptr[6]=area.ptr[7]=area.ptr[8]=1.0;
	  r_base.ptr[0]=ptr[0]; //initialize
      for(i=1; i<nl; i++)    //find the starting point with lowest y, and put it into 1st row
         if(ptr[i*3+2]<ptr[2]){
            r_base.ptr[0]=ptr[i*3+0]; // label of point A is stored in p[0]
            exrow(1,i+1);             // A is stored in the 1st row
 	      }
     area.ptr[0]=ptr[1];    //the 1st column of matrix area stores the determined hull point A
     area.ptr[3]=ptr[2];
     for(i=1; i<nl; i++){
         if((ptr[i*3+1]==ptr[0*3+1]) && (ptr[i*3+2]==ptr[0*3+2])) continue;//if coordinate of B= coordinate of A,B is not OK
	      h=true;
         area.ptr[1]=ptr[i*3+1]; //the 2nd column of matrix area stores the hull point candidate B
         area.ptr[4]=ptr[i*3+2];
        // cout<<"candidate is B ("<<ptr[i*3+0]<<")"<<endl;
         for(j=0; j<nl; j++){
	         if((j==i)||(j==0)) continue;
                 if((ptr[j*3+1]==ptr[i*3+1]) && (ptr[j*3+2]==ptr[i*3+2])) continue; //if coord of C = coord of B, C is not OK
                 if((ptr[j*3+1]==ptr[0*3+1]) && (ptr[j*3+2]==ptr[0*3+2])) continue; //if coord of C = coord of A, C is not OK
            area.ptr[2]=ptr[j*3+1]; //the 3rd column of matrix area stores the third point C
            area.ptr[5]=ptr[j*3+2];
          // cout<<"candidate is C ("<<ptr[j*3+0]<<")"<<endl;
            area_value=area.det();
          //  cout<<"area matrix is "<<endl<<area<<endl<<"area= "<<area_value<<endl;
           // cin>>screen;
            if (fabs(area_value)<1.0e-12) continue;
            else if(area_value<0) {h=false;break;} // if for all other points except A and B, the area is positive or zero
            else continue;
	      }                                     // that means all points C is to the left of line AB, then keep B as a
		                                       // hull point
   ...
   if(cand_size==1) {
	  ...
	  return r_base;
       ...
       for(i=0; i<cand_size; i++){
	      ...
	      if(distance>pre_distance) {r_base.ptr[1]=temp2d.ptr[0]; pre_distance=distance;}
		  cand.pop();
	   }
    //cout<<"2D hull base is: "<<r_base<<endl;
    return r_base;
      ...
      for(i=0; i<nl; i++)
          for(j=0; j<n; j++)
              reduce.ptr[i*n+j]=ptr[i*(n+1)+j];
      re_h_base=reduce.hull_base(n-1);//call hull_base(2)
      cout<<re_h_base<<endl;     
/////////////////////////////////////////////////////////////////////////////////////////////
      //what the hull_base(2) find may be not an good edge, because it just consider x and y coordinates, not z
      double x_y_z[4]={-1000,-1000,-1000,-1000};
      // select the coordinates and label of the 1st point on the edge returned by hull_base(2)
      for(i=0; i<nl; i++){
         if(ptr[i*4+0] == re_h_base.ptr[0]){//if label equal
           x_y_z[0] = ptr[i*4+0]; x_y_z[1] = ptr[i*4+1]; x_y_z[2] = ptr[i*4+2]; x_y_z[3] = ptr[i*4+3];
           break;
         }
      }
      for(i=0; i<nl; i++){//find the point with smallest z, stored in x_y_z[]
         if(ptr[i*4+0] == re_h_base.ptr[0]) continue;
         if(ptr[i*4+1]!=x_y_z[1] || ptr[i*4+2]!= x_y_z[2]) continue;  //only select points with the same x,y with 1st point
         if(ptr[i*4+3] < x_y_z[3]) {
           x_y_z[0]=ptr[i*4+0]; x_y_z[1] = ptr[i*4+1]; x_y_z[2] = ptr[i*4+2]; x_y_z[3] = ptr[i*4+3];
         }
      }  
      re_h_base.ptr[0] = x_y_z[0];   
      //select the coordinates and label of the 2nd point on the edge returned by hull_base(2)
      for(i=0; i<nl; i++){
         if(ptr[i*4+0] == re_h_base.ptr[1]){
           x_y_z[0] = ptr[i*4+0]; x_y_z[1] = ptr[i*4+1]; x_y_z[2] = ptr[i*4+2]; x_y_z[3] = ptr[i*4+3];
           break;
         }
      }
      for(i=0; i<nl; i++){//find the point with smallest z, stored in x_y_z[]
         if(ptr[i*4+0]==re_h_base.ptr[1]) continue;
         if(ptr[i*4+1]!=x_y_z[1] || ptr[i*4+2]!= x_y_z[2]) continue;  //only select points with the same x,y with 1st point
         if(ptr[i*4+3] < x_y_z[3]) {
           x_y_z[0]=ptr[i*4+0]; x_y_z[1] = ptr[i*4+1]; x_y_z[2] = ptr[i*4+2]; x_y_z[3] = ptr[i*4+3];
         }
      }
      re_h_base.ptr[1] = x_y_z[0];
//////////////////////////////////// ///
      for(j=0; j<n-1; j++)
          r_base.ptr[j]=re_h_base.ptr[j];
      for(j=0; j<n-1; j++)
      ...
      for(i=n-1; i<nl; i++){
         ...
         for(j=0; j<nl; j++){
            ...
            for(s=0; s<n; s++)          // the last column of deter matrix
                deter.ptr[s*(n+1)+n]=ptr[j*(n+1)+s+1];
            deter_value=deter.det();
            if(fabs(deter_value)<1.0e-12){count--; continue;}
            else if(deter_value<0) sign=-1;
            else if(deter_value>0) sign=1;
            else {count--; continue;}
            if (count==1){pre_sign=sign; continue;}
   ...
   if(cand_3d_size==1){
	  ...
	  for(i=0; i<cand_3d_size; i++){
	    ...
	    if(distance_3d>pre_distance_3d) {r_base.ptr[2]=temp3d.ptr[0];pre_distance_3d=distance_3d;}
	    cand_3d.pop();
	  }  	 	 
      return r_base;
    }	  
...
{int  mark, screen;
 ...
 for(i=0; i<n; i++){
     for(j=0; j<nc; j++)
         tran[j]=ptr[i*nc+j];   // 1st 3 rows of ptr
     tranA.setArray(tran);
...
for(i=0; i<edge_size; i++){
    temp_edge=edge.front();//select an edge 
...
if(num_appear>=2) {;}//cout<<"1st edge is 2 times"<<endl
	
else {new_elem.push(tranA);new_elem.push(tranA_2);num_edge++;}             //push one edge into queue
...
if(num_appear>=2) {;}//cout<<"2nd edge is 2 times"<<endl;} 	
else {new_elem.push(tranA);new_elem.push(tranA_2);num_edge++;}             
...
if(num_appear>=2) {;}//cout<<"3rd edge is 2 times"<<endl;} 	
else {new_elem.push(tranA);new_elem.push(tranA_2);num_edge++;}             

if(new_elem.empty()) { facet.pop();continue;}
//cout<<num_edge<<endl;	 
for(new_i=0; new_i<num_edge; new_i++){    //get one edge each time
 ...
 while(!remain_queue.empty()){      //1
	 ...
	 if((tranA.ptr[0]==facetA.ptr[0])||(tranA.ptr[0]==facetA.ptr[1])||(tranA.ptr[0]==facetA.ptr[2])) {remain_queue.pop();continue;}
	 theta=((sub_sys[1]-tranA.ptr[1])*(sub_sys[5]-tranA.ptr[1])+(sub_sys[2]-tranA.ptr[2])*(sub_sys[6]-tranA.ptr[2])+(sub_sys[7]-tranA.ptr[3])*(sub_sys[3]-tranA.ptr[3]))/
		      (sqrt((sub_sys[1]-tranA.ptr[1])*(sub_sys[1]-tranA.ptr[1])+(sub_sys[2]-tranA.ptr[2])*(sub_sys[2]-tranA.ptr[2])+(sub_sys[3]-tranA.ptr[3])*(sub_sys[3]-tranA.ptr[3]))*
			  sqrt((sub_sys[5]-tranA.ptr[1])*(sub_sys[5]-tranA.ptr[1])+(sub_sys[6]-tranA.ptr[2])*(sub_sys[6]-tranA.ptr[2])+(sub_sys[7]-tranA.ptr[3])*(sub_sys[7]-tranA.ptr[3])) );
	  if((theta==1.0)||(theta==-1.0)){
     ...
     for(j=0; j<nl; j++){
         ...
         //cout<<"four points are: "<<label[0]<<"  "<<label[1]<<"  "<<label[2]<<"  "<<ptr[j*4+0]<<endl;
         
         deter2A.setArray(deter2);
         deter2_value=deter2A.det();
         //cout<<"the deter2A = "<<endl<<deter2A<<endl;
         //cout<<"deter value is: "<<deter2_value<<endl;
         //cin>>screen;
         if(fabs(deter2_value)<1.0e-12) {count--; continue;}
	 else if(deter2_value<0) sign=-1;
         else if(deter2_value>0) sign=1;
         else {count--; continue;}
         if (count==1){pre_sign=sign; continue;}
	...
	//	    cout<<"point "<<tranA.ptr[0]<<" on_plane"<<endl;            //if p4 is on the plane p1-p2-p3
		    if(in_tri(p4,p1,p2,p3)) {remain_queue.pop();continue;}  //if p4 is within triangle p1-p2-p3
			else if(same_side(p4,p3,p1,p2)) {remain_queue.pop(); continue;}//if p4 and p3 are to the same side of edge p1-p2
			else{};
		 }
		...
		cut_value=false; 
		for(i=0; i<2; i++){
		 if(i==0) {p11=p1;p22=p4;}    //get one new edge;
		 else {p11=p2;p22=p4;}        //get another new edge;
		 ...
		 for(pp=0; pp<cut_facetA_size; pp++){
		    ...
		    if(cut(p33,p44,p55,p11,p22)) {cut_value=true;break;} //if the new edge cut one existing facet
			cut_facet.push(cut_facetA);
			cut_facet.pop();
		}
		if(cut_value==true) break;
	  }						
	  if(cut_value==true) {remain_queue.pop(); continue;}	   		   			   
		 /////////////////
		 
		 dis=distance_pl(p4,p1,p2);            //calculate the distance between point p4 and edge p1-p2
		 if(dis>pre_dis) {keep=tranA; pre_dis=dis;remain_queue.pop();}
		 else {remain_queue.pop();}
	  }	 
	}//1
...
//	cout<<"the obtained facet is "<<h_base<<endl;
  //      cin>>screen;	
	...
	for(i=0; i<hull_queue_size; i++){
		...
		if(tranA.ptr[0]==keep.ptr[0]) {same_point=true; break;}
		else{hull_queue.push(tranA); hull_queue.pop();}
	}
	if(same_point==false) {hull_queue.push(keep); num_hull++;} 
 }
 facet.pop();
}
...
for(i=0; i<nl; i++){                   
    ...
    for(j=0; j<num_hull; j++){
	    tranA=hull_queue.front();
		if(ptr[i*4+0]==tranA.ptr[0]) {hull_queue.push(tranA); hull_queue.pop();same=true; break;}
		else {hull_queue.push(tranA); hull_queue.pop();}
	}
	if (same==false){ 
   ...
   for(pp=0; pp<remain_size; pp++){
      ...
      if(!in_tri(p4,p1,p2,p3)){             // the point is not on the facet
		 ...
		 continue;				// get the next point from remain queue
		   ...
		   cout<<"Point "<<setw(4)<<tranA.ptr[0]<<"  is on facet  "<<h_base;
           num_hull++;
		   remain_queue.pop();              // delete the point from the remain queue
	  }
   }
...
for(m=1; m<nl; m++){
     ...
     for(i=0; i<nl; i++){      //find the hull point B associated with A
         ...
         for(j=0; j<nl; j++){
            ...
            if(deter2A.det()<0) {h=false;break;} // if for all other points except A and B, the area is positive or zero
	     }                                     // that means all points C is to the left of line AB, then keep B as a 
										       // hull point    
	    if (h==true) {
		    h_out=true;
		    dis=sqrt((ptr[i*3+1]-ptr[m*3+1])*(ptr[i*3+1]-ptr[m*3+1])+(ptr[i*3+2]-ptr[m*3+2])*(ptr[i*3+2]-ptr[m*3+2]));
			if(dis>pre_dis) {pre_dis=dis; keep_label=i;keep.ptr[0]=ptr[i*3+0]; keep.ptr[1]=ptr[i*3+1]; keep.ptr[2]=ptr[i*3+2]; continue;}
			else{continue;}
		 }
	  }	 	
	  if(h_out==true){
	    if(keep.ptr[0]==ptr[0]) {edgeA.ptr[1]=ptr[0]; edge.push(edgeA); num_edge++; break;}	
		else {
	  ...
	  else {edgeA.ptr[1]=ptr[0]; edge.push(edgeA); num_edge++;break;}   	
  }
...
//////////////////////////////////////////////////////////////////////////////////
else {cout<<" the dimension is not 2 or 3!"<<endl; exit (1);}
}
//********* calculate hull points and facets on the bottom half hull for 3D **********************
Array Array::half_hull3(Array &fac ) //the facet list is stored in fac, and hull point list is the returned result
{// half_hull3  BEGIN                //the input array includes label and coordinates of all points and has dimesion of(num of points, 4)
 Array ini_hullp, ini_fac;
 ini_hullp = hullnd(3, ini_fac);
 //cout<<endl<<ini_hullp<<endl; cout<<endl<<ini_fac<<endl;
 Array D(1, 3);   // determine one point D inside the hull
 int i, j, full;
 bool is_hull;
 
 //determine a point D that is inside the hull
 for(i=0; i<nl; i++){  // select one point from total point list
	is_hull = false;   // assume the selected point is not hull
    for(j=0; j<ini_hullp.nl; j++)  //select one point from hull point list
	   if(ptr[i*4+0] == ini_hullp.ptr[j*4+0]) {is_hull = true; break;}   //if the selected point is a hull point, don't select it
	if(is_hull == false){ D.ptr[0] = ptr[i*4+1]; D.ptr[1] = ptr[i*4+2]; D.ptr[2] = ptr[i*4+3]; break;} 
 }	
 
 //determine the facets on the bottom half hull
 Array deter(4,4);
 Array trans_facet(1,3), trans_point(1,4);
 queue <Array> bottom_facet; //store facet on the bottom half hull
 double deter_value, crossZ;
 deter.ptr[12] = deter.ptr[13] = deter.ptr[14] = deter.ptr[15] = 1.0;  //initialize last row of deter matrix
 deter.ptr[3]=D.ptr[0] ; deter.ptr[7]=D.ptr[1]; deter.ptr[11]=D.ptr[2]; //put the coordinates of D into last column of deter matrix
 for(i=0; i<ini_fac.nl; i++){  // select a facet LOOP1
    full=0;
    for(j=0; j<ini_hullp.nl; j++){  //determing the coordinates of facet corners, put them into deter matrix
	   if(ini_hullp.ptr[j*4+0] == ini_fac.ptr[i*3+0]) {deter.ptr[0]=ini_hullp.ptr[j*4+1] ; deter.ptr[4]=ini_hullp.ptr[j*4+2]; deter.ptr[8]=ini_hullp.ptr[j*4+3]; full++;} // point A
	   else if(ini_hullp.ptr[j*4+0] == ini_fac.ptr[i*3+1]) {deter.ptr[1]=ini_hullp.ptr[j*4+1] ; deter.ptr[5]=ini_hullp.ptr[j*4+2]; deter.ptr[9]=ini_hullp.ptr[j*4+3]; full++;} // point B
	   else if(ini_hullp.ptr[j*4+0] == ini_fac.ptr[i*3+2]) {deter.ptr[2]=ini_hullp.ptr[j*4+1] ; deter.ptr[6]=ini_hullp.ptr[j*4+2]; deter.ptr[10]=ini_hullp.ptr[j*4+3]; full++;} // point C
           else {;}
           if(full==3) break;
	}
    deter_value = deter.det(); // calculate determinate of deter matrix
	crossZ = (deter.ptr[1]-deter.ptr[0])*(deter.ptr[6]-deter.ptr[4]) - (deter.ptr[2]-deter.ptr[0])*(deter.ptr[5]-deter.ptr[4]);// calculate z component of ABxAC cross product
	if(deter_value > 0.0){                 //ABC is counter-clockwise from outside view
	  if(fabs(crossZ) < 1.0e-13) continue; //if the facet is parallel to the z axis, discard it
	  else if(crossZ > 0.0) continue;      //if the facet is on top half hull, discard it
	  else {trans_facet.ptr[0]=ini_fac.ptr[i*3+0]; trans_facet.ptr[1]=ini_fac.ptr[i*3+1]; trans_facet.ptr[2]=ini_fac.ptr[i*3+2];
	        bottom_facet.push(trans_facet);}            //if the facet is on bottom half hull, keep it
	}
	else{  //ABC is clockwise from outside view
	  if(fabs(crossZ)< 1.0e-13) continue; //if the facet is parallel to the z axis, discard it
	  else if(crossZ < 0.0) continue;     //if the facet is on the top half hull, discard it
	  else {trans_facet.ptr[0]=ini_fac.ptr[i*3+0]; trans_facet.ptr[1]=ini_fac.ptr[i*3+1]; trans_facet.ptr[2]=ini_fac.ptr[i*3+2];
	        bottom_facet.push(trans_facet);}            //if the facet is on bottom half hull, keep it
	}
 }               //LOOP 1
 Array final_fac(bottom_facet.size(), 3);  //store selected facet
 
 for(i=0; i<final_fac.nl; i++){
    trans_facet = bottom_facet.front();
	final_fac.ptr[i*3+0] = trans_facet.ptr[0];
	final_fac.ptr[i*3+1] = trans_facet.ptr[1];
	final_fac.ptr[i*3+2] = trans_facet.ptr[2];
	bottom_facet.pop();
 }
 fac = final_fac;
 
 //determing the hull points on the bottom half hull            
 queue <Array> bottom_hullp; //store hull points on the bottom hull
 for(i=0; i<ini_hullp.nl; i++){   // LOOP 2, determine hull points that are on the bottom facet one by one
    for(j=0; j<final_fac.nl; j++){  //select a facet from the OK facets LOOP3 to see if point is on the OK facet
           full=0;
	   if((ini_hullp.ptr[i*4+0] == final_fac.ptr[j*3+0]) || (ini_hullp.ptr[i*4+0] == final_fac.ptr[j*3+1]) || (ini_hullp.ptr[i*4+0] == final_fac.ptr[j*3+2]) )
	     {
		  trans_point.ptr[0] = ini_hullp.ptr[i*4+0]; trans_point.ptr[1] = ini_hullp.ptr[i*4+1];
		  trans_point.ptr[2] = ini_hullp.ptr[i*4+2]; trans_point.ptr[3] = ini_hullp.ptr[i*4+3];
		  bottom_hullp.push(trans_point);
		  //cout<<"find a point "<<trans_point<<endl;
		  break;
		 }                          // if the hull point is a corner of the OK facet, then keep it
	   	 deter.ptr[3]=ini_hullp.ptr[i*4+1] ; deter.ptr[7]=ini_hullp.ptr[i*4+2]; deter.ptr[11]=ini_hullp.ptr[i*4+3];
		 for(int k=0; k<ini_hullp.nl; k++){  //determing the coordinates of facet corners, put them into deter matrix
	        if(ini_hullp.ptr[k*4+0] == final_fac.ptr[j*3+0]) {deter.ptr[0]=ini_hullp.ptr[k*4+1] ; deter.ptr[4]=ini_hullp.ptr[k*4+2]; deter.ptr[8]=ini_hullp.ptr[k*4+3]; full++;} // point A 
	        else if(ini_hullp.ptr[k*4+0] == final_fac.ptr[j*3+1]) {deter.ptr[1]=ini_hullp.ptr[k*4+1] ; deter.ptr[5]=ini_hullp.ptr[k*4+2]; deter.ptr[9]=ini_hullp.ptr[k*4+3]; full++;} // point B
	        else if(ini_hullp.ptr[k*4+0] == final_fac.ptr[j*3+2]) {deter.ptr[2]=ini_hullp.ptr[k*4+1] ; deter.ptr[6]=ini_hullp.ptr[k*4+2]; deter.ptr[10]=ini_hullp.ptr[k*4+3];full++;} // point C
			else {;}
			if(full==3) break;
		 }
         deter_value = deter.det(); // calculate determinate of deter matrix
         if(fabs(deter_value)<1.0e-13){
            trans_point.ptr[0]=ini_hullp.ptr[i*4+0];
            trans_point.ptr[1]=ini_hullp.ptr[i*4+1];
            trans_point.ptr[2]=ini_hullp.ptr[i*4+2];
            trans_point.ptr[3]=ini_hullp.ptr[i*4+3];
            bottom_hullp.push(trans_point);
			//cout<<"find a point "<<trans_point<<endl;
            break;
         }
 ...
 //cout<<"size of bottom_hullp is: "<<bottom_hullp.size()<<endl;
 Array final_hp(bottom_hullp.size(), 4);
 for(i=0; i<final_hp.nl; i++){
    trans_point=bottom_hullp.front();
    final_hp.ptr[i*4+0]=trans_point.ptr[0];
    final_hp.ptr[i*4+1]=trans_point.ptr[1];
    final_hp.ptr[i*4+2]=trans_point.ptr[2];
    final_hp.ptr[i*4+3]=trans_point.ptr[3];
    bottom_hullp.pop();
 }
 return final_hp;
}// half_hull3  END
