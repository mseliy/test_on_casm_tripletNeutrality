...
class Monte_Carlo{
 public:
  ...
  void grand_canonical_MgV2O5(double beta, chempot mu, int n_pass, int n_equil_pass); //Added by Aziz
...
void Monte_Carlo::grand_canonical_MgV2O5(double beta, chempot mu, int n_pass, int n_equil_pass){


//This subroutine performs Monte Carlo simulations in the specific case of MgxV2O5. This subroutine only allows
//moves that preserve charge neutrality

//The Hamiltonian in this routine is a bulk cluster expansion

//It is assumed that the basis indices b=0 to 3 point to Mg/Va sites (adjust PRIM file accordingly)
//while the basis indices b=4 to 11 point to electron/holes (adjust PRIM file accordingly)

//It is assumed that the PRIM file reads Mg and electron/hole


  int i_Mg,j_Mg,k_Mg,b_Mg,l_Mg; //Position indices for Mg/Va 
  int i_electron_1,j_electron_1,k_electron_1,b_electron_1,l_electron_1; //Position indices of the first electron/hole site
  int i_electron_2,j_electron_2,k_electron_2,b_electron_2,l_electron_2; //Position indices of the second electron/hole site
  ...
  //Calculating the grand energy
  double grand_energy=calc_grand_canonical_energy(mu);

  cout << "grand canonical energy = " << grand_energy/nuc << "\n"; //Normalized per unit cell

  bool right_combination; //Variable used to determine if a valid combination of sites has been selected (Mg vacancy and 2 holes or Mg+ and 2 electrons)

  for(int n=0; n<n_pass; n++){
    //pick nmcL electron/hole pairs or Mg pairs at random
    for(int nn=0; nn<nmcL; nn++){

      //Loop until either a O/Ti pair or N/Ta pair is selected
      ...
      while (right_combination==false){
        i_Mg=int(di*ran0(idum));
        j_Mg=int(dj*ran0(idum));
        k_Mg=int(dk*ran0(idum));
        b_Mg=int(4*ran0(idum));  //It is assumed that, in the PRIM file, the indices b=0 to 9 point to Mg atoms 
        l_Mg=index(i_Mg,j_Mg,k_Mg,b_Mg);

        i_electron_1=int(di*ran0(idum));
        j_electron_1=int(dj*ran0(idum));
        k_electron_1=int(dk*ran0(idum));
        b_electron_1=int(8*ran0(idum)+4);  //It is assumed that, in the PRIM file, the indices b=10 to 15 point to electron/hole 
        l_electron_1=index(i_electron_1,j_electron_1,k_electron_1,b_electron_1);
        
        i_electron_2=int(di*ran0(idum));
        j_electron_2=int(dj*ran0(idum));
        k_electron_2=int(dk*ran0(idum));
        b_electron_2=int(8*ran0(idum)+4);  //It is assumed that, in the PRIM file, the indices b=10 to 15 point to electron/hole 
        l_electron_2=index(i_electron_2,j_electron_2,k_electron_2,b_electron_2);

       //Testing if a valid combination has been identified (Mg with 2 electrons or Va with two holes)
	   if (mcL[l_electron_1] == mcL[l_Mg] && mcL[l_electron_2] == mcL[l_Mg] && l_electron_1!=l_electron_2){
      ...
      //Determine the index of the current occupant at sites l_Mg, l_electron_1 and l_electron_2  
      int co_Mg=basis[b_Mg].iflip(mcL[l_Mg]);
	  int co_electron_1=basis[b_electron_1].iflip(mcL[l_electron_1]);
	  int co_electron_2=basis[b_electron_2].iflip(mcL[l_electron_2]);

      //pick a flip event for Mg/Va and electron/hole
      int f_Mg=int(ran0(idum)*basis[b_Mg].flip[co_Mg].size());
	  int f_electron_1=int(ran0(idum)*basis[b_electron_1].flip[co_electron_1].size());
	  int f_electron_2=int(ran0(idum)*basis[b_electron_2].flip[co_electron_2].size());
      int tspin_Mg=mcL[l_Mg];
	  int tspin_electron_1=mcL[l_electron_1];
	  int tspin_electron_2=mcL[l_electron_2];

	  //To calculate the difference in energy due to the two flips, we will first perform
	  // an Mg/Va flip and then the electron/hole flips

	  //Mg flip
      //get point energy before the Mg spin flip
      double en_before_flip_Mg=pointenergy(i_Mg,j_Mg,k_Mg,b_Mg)-basis[b_Mg].mu[co_Mg];

      //get point energy after the Mg spin flip
      mcL[l_Mg]=basis[b_Mg].flip[co_Mg][f_Mg];
      int no_Mg=basis[b_Mg].iflip(mcL[l_Mg]);
      double en_after_flip_Mg=pointenergy(i_Mg,j_Mg,k_Mg,b_Mg)-basis[b_Mg].mu[no_Mg];

      double delta_energy_flip_Mg;
      delta_energy_flip_Mg=en_after_flip_Mg-en_before_flip_Mg;


	  // electron/hole flip
	  
      //get point energy before the first electron/hole spin flip
      double en_before_flip_electron_1=pointenergy(i_electron_1,j_electron_1,k_electron_1,b_electron_1)-basis[b_electron_1].mu[co_electron_1];

      //get point energy after the first electron/hole flip
      mcL[l_electron_1]=basis[b_electron_1].flip[co_electron_1][f_electron_1];
      int no_electron_1=basis[b_electron_1].iflip(mcL[l_electron_1]);
      double en_after_flip_electron_1=pointenergy(i_electron_1,j_electron_1,k_electron_1,b_electron_1)-basis[b_electron_1].mu[no_electron_1];

      double delta_energy_flip_electron_1;
      delta_energy_flip_electron_1=en_after_flip_electron_1-en_before_flip_electron_1;
      
      //get point energy before the second electron/hole spin flip
      double en_before_flip_electron_2=pointenergy(i_electron_2,j_electron_2,k_electron_2,b_electron_2)-basis[b_electron_2].mu[co_electron_2];

      //get point energy after the first electron/hole flip
      mcL[l_electron_2]=basis[b_electron_2].flip[co_electron_2][f_electron_2];
      int no_electron_2=basis[b_electron_2].iflip(mcL[l_electron_2]);
      double en_after_flip_electron_2=pointenergy(i_electron_2,j_electron_2,k_electron_2,b_electron_2)-basis[b_electron_2].mu[no_electron_2];

      double delta_energy_flip_electron_2;
      delta_energy_flip_electron_2=en_after_flip_electron_2-en_before_flip_electron_2;

	  //Calculating the total change in energy
	  double delta_energy=delta_energy_flip_Mg+delta_energy_flip_electron_1+delta_energy_flip_electron_2;

	  //Applying the Metropolis condition
      if(delta_energy < 0.0 || exp(-delta_energy*beta) >=ran0(idum)){
        flipfreq++;
        grand_energy=grand_energy+delta_energy;
      }
        ...
        mcL[l_Mg]=tspin_Mg;
	    mcL[l_electron_1]=tspin_electron_1;
	    mcL[l_electron_2]=tspin_electron_2;
      }
    }

	//Calculate the correlations
    //(Note : We have to check if these correlations things still apply to the simultaneous flip case)
    ...
    if(n >= n_equil_pass){
	  ...
	  AVenergy=AVenergy+grand_energy; //The grand energy is eventually normalized
      heatcap=heatcap+(grand_energy*grand_energy); //The heat capacity is eventually normalized
      calc_num_atoms();
      calc_concentration();
      calc_sublat_concentration();
      AVconc.increment(conc);
  ...
  heatcap=heatcap/(n_pass-n_equil_pass);
  heatcap=(heatcap-(AVenergy*AVenergy))*(beta*beta)*kb;  //Final value of the heat capacity
  AVconc.normalize(n_pass-n_equil_pass);
  AVsublat_conc.normalize(n_pass-n_equil_pass);
  AVnum_atoms.normalize(n_pass-n_equil_pass);
  uncorr_susc.evaluate(AVnum_atoms);
...
double Monte_Carlo::pointenergy(int i, int j, int k, int b){
  ...
  if(b == 0){
     ...
     l=index(i-1,j-1,k,8);
     int p210=mcL[l];
     l=index(i-1,j-1,k,11);
     int p220=mcL[l];
     l=index(i+1,j-1,k,8);
     int p230=mcL[l];
     l=index(i+1,j-1,k,11);
     int p240=mcL[l];
     l=index(i-2,j,k-1,10);
     int p250=mcL[l];
     l=index(i-2,j,k,9);
     int p260=mcL[l];
     l=index(i+2,j,k-1,10);
     ...
     energy = energy+0.0705851*(p00)-0.0226488*(p00*p10+p20*p00)-0.0529272*(p00*p30+p40*p00+p00*p50+p60*p00)-0.00450121*(p00*p70+p00*p80+p00*p90+p00*p100)-0.0089906*(p110*p00+p120*p00)-0.00438516*(p00*p130+p00*p140)+0.0042292*(p00*p150+p00*p160)+0.00704984*(p00*p170+p00*p180+p00*p190+p00*p200)-0.0025048*(p00*p210+p00*p220+p00*p230+p00*p240)-0.00600997*(p00*p250+p00*p260+p00*p270+p00*p280)-0.00999758*(p00*p290*p300+p00*p310*p320)-0.00264485*(p00*p330*p340+p00*p350*p360+p00*p330*p370+p00*p350*p380)-0.0045403*(p00*p300*p370+p00*p290*p380+p00*p320*p340+p00*p310*p360)+0.00319095*(p290*p100*p00+p300*p90*p00+p310*p80*p00+p320*p70*p00)-0.00404378*(p120*p390*p00+p110*p400*p00+p00*p330*p110+p00*p350*p120);
     return energy;
  }
  ...
  if(b == 1){
     ...
     energy = energy+0.0705851*(p00)-0.0226488*(p00*p10+p20*p00)-0.0529272*(p00*p30+p40*p00+p00*p50+p60*p00)-0.00450121*(p00*p70+p00*p80+p00*p90+p00*p100)-0.0089906*(p00*p110+p00*p120)-0.00438516*(p00*p130+p00*p140)+0.0042292*(p00*p150+p00*p160)+0.00704984*(p00*p170+p00*p180+p00*p190+p00*p200)-0.0025048*(p00*p210+p00*p220+p00*p230+p00*p240)-0.00600997*(p00*p250+p00*p260+p00*p270+p00*p280)-0.00999758*(p00*p290*p300+p00*p310*p320)-0.00264485*(p00*p330*p340+p00*p350*p360+p00*p330*p370+p00*p350*p380)-0.0045403*(p00*p300*p370+p00*p290*p380+p00*p320*p340+p00*p310*p360)+0.00319095*(p290*p100*p00+p300*p90*p00+p310*p80*p00+p320*p70*p00)-0.00404378*(p00*p330*p120+p00*p350*p110+p110*p390*p00+p120*p400*p00);
     return energy;
  }
  ...
  if(b == 2){
     ...
     l=index(i,j,k,6);
     int p160=mcL[l];
     l=index(i-2,j+1,k-1,10);
     int p170=mcL[l];
     l=index(i-2,j+1,k,9);
     int p180=mcL[l];
     l=index(i+1,j+1,k-1,10);
     int p190=mcL[l];
     l=index(i+1,j+1,k,9);
     int p200=mcL[l];
     l=index(i-1,j,k,4);
     ...
     energy = energy+0.0705851*(p00)-0.0226488*(p00*p10+p20*p00)-0.0529272*(p30*p00+p00*p40+p50*p00+p00*p60)-0.00450121*(p00*p70+p00*p80+p00*p90+p00*p100)-0.0089906*(p110*p00+p120*p00)-0.00438516*(p00*p130+p00*p140)+0.0042292*(p00*p150+p00*p160)+0.00704984*(p00*p170+p00*p180+p00*p190+p00*p200)-0.0025048*(p00*p210+p00*p220+p00*p230+p00*p240)-0.00600997*(p00*p250+p00*p260+p00*p270+p00*p280)-0.00999758*(p00*p290*p300+p00*p310*p320)-0.00264485*(p00*p330*p340+p00*p350*p360+p00*p330*p370+p00*p350*p380)-0.0045403*(p00*p300*p370+p00*p290*p380+p00*p320*p340+p00*p310*p360)+0.00319095*(p290*p100*p00+p300*p90*p00+p310*p80*p00+p320*p70*p00)-0.00404378*(p120*p390*p00+p110*p400*p00+p00*p330*p110+p00*p350*p120);
     return energy;
  }
  ...
  if(b == 3){
     ...
     l=index(i-2,j-1,k,8);
     int p170=mcL[l];
     l=index(i-2,j-1,k+1,11);
     int p180=mcL[l];
     l=index(i+1,j-1,k,8);
     int p190=mcL[l];
     l=index(i+1,j-1,k+1,11);
     int p200=mcL[l];
     l=index(i-1,j,k,6);
     int p210=mcL[l];
     l=index(i-1,j,k,5);
  ...
  if(b == 4){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p00*p20+p30*p00)+0.00440368*(p00*p40+p50*p00)-0.00450121*(p60*p00+p70*p00)-0.00532187*(p80*p00+p00*p90)-0.00438516*(p100*p00)+0.0042292*(p110*p00)+0.00704984*(p120*p00+p130*p00)-0.0025048*(p140*p00+p150*p00)-0.00684347*(p00*p160+p170*p00)-0.00600997*(p180*p00+p190*p00)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.00264485*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.0045403*(p200*p00*p250+p240*p260*p00+p210*p00*p270+p230*p280*p00)+0.00319095*(p290*p00*p70+p00*p300*p200+p300*p00*p60+p00*p290*p210)-0.00210824*(p310*p90*p00+p00*p20*p310+p30*p00*p80+p310*p80*p00+p00*p30*p310+p20*p00*p90)-0.00404378*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 5){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p10*p20+p30*p10)+0.00440368*(p40*p10+p10*p50)-0.00450121*(p60*p10+p70*p10)-0.00532187*(p10*p80+p90*p10)-0.00438516*(p100*p10)+0.0042292*(p110*p10)+0.00704984*(p120*p10+p130*p10)-0.0025048*(p140*p10+p150*p10)-0.00684347*(p160*p10+p10*p170)-0.00600997*(p180*p10+p190*p10)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.00264485*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.0045403*(p200*p10*p250+p240*p260*p10+p210*p10*p270+p230*p280*p10)+0.00319095*(p10*p290*p200+p300*p10*p70+p10*p300*p210+p290*p10*p60)-0.00210824*(p10*p20*p310+p30*p10*p90+p310*p80*p10+p10*p30*p310+p20*p10*p80+p310*p90*p10)-0.00404378*(p220*p10*p320);
     return energy;
  }
  ...
  if(b == 6){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p00*p20+p30*p00)+0.00440368*(p00*p40+p50*p00)-0.00450121*(p60*p00+p70*p00)-0.00532187*(p00*p80+p90*p00)-0.00438516*(p100*p00)+0.0042292*(p110*p00)+0.00704984*(p120*p00+p130*p00)-0.0025048*(p140*p00+p150*p00)-0.00684347*(p00*p160+p170*p00)-0.00600997*(p180*p00+p190*p00)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.00264485*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.0045403*(p200*p00*p250+p240*p260*p00+p210*p00*p270+p230*p280*p00)+0.00319095*(p00*p290*p200+p300*p00*p70+p00*p300*p210+p290*p00*p60)-0.00210824*(p00*p20*p310+p30*p00*p90+p310*p80*p00+p00*p30*p310+p20*p00*p80+p310*p90*p00)-0.00404378*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 7){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p10*p20+p30*p10)+0.00440368*(p40*p10+p10*p50)-0.00450121*(p60*p10+p70*p10)-0.00532187*(p80*p10+p10*p90)-0.00438516*(p100*p10)+0.0042292*(p110*p10)+0.00704984*(p120*p10+p130*p10)-0.0025048*(p140*p10+p150*p10)-0.00684347*(p160*p10+p10*p170)-0.00600997*(p180*p10+p190*p10)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.00264485*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.0045403*(p200*p10*p250+p240*p260*p10+p210*p10*p270+p230*p280*p10)+0.00319095*(p290*p10*p70+p10*p300*p200+p300*p10*p60+p10*p290*p210)-0.00210824*(p310*p90*p10+p10*p20*p310+p30*p10*p80+p310*p80*p10+p10*p30*p310+p20*p10*p90)-0.00404378*(p220*p10*p320);
     return energy;
  }
  ...
  if(b == 8){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p00*p20+p30*p00)+0.00440368*(p00*p40+p50*p00)-0.00450121*(p60*p00+p70*p00)-0.00532187*(p80*p00+p00*p90)-0.00438516*(p100*p00)+0.0042292*(p110*p00)+0.00704984*(p120*p00+p130*p00)-0.0025048*(p140*p00+p150*p00)-0.00684347*(p00*p160+p170*p00)-0.00600997*(p180*p00+p190*p00)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.00264485*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.0045403*(p240*p250*p00+p200*p00*p260+p230*p270*p00+p210*p00*p280)+0.00319095*(p290*p00*p70+p00*p300*p200+p300*p00*p60+p00*p290*p210)-0.00210824*(p310*p90*p00+p00*p20*p310+p30*p00*p80+p310*p80*p00+p00*p30*p310+p20*p00*p90)-0.00404378*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 9){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p10*p20+p30*p10)+0.00440368*(p40*p10+p10*p50)-0.00450121*(p60*p10+p70*p10)-0.00532187*(p10*p80+p90*p10)-0.00438516*(p100*p10)+0.0042292*(p110*p10)+0.00704984*(p120*p10+p130*p10)-0.0025048*(p140*p10+p150*p10)-0.00684347*(p160*p10+p10*p170)-0.00600997*(p180*p10+p190*p10)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.00264485*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.0045403*(p240*p250*p10+p200*p10*p260+p230*p270*p10+p210*p10*p280)+0.00319095*(p10*p290*p200+p300*p10*p70+p10*p300*p210+p290*p10*p60)-0.00210824*(p10*p20*p310+p30*p10*p90+p310*p80*p10+p10*p30*p310+p20*p10*p80+p310*p90*p10)-0.00404378*(p220*p10*p320);
     return energy;
  }
  ...
  if(b == 10){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p00*p20+p30*p00)+0.00440368*(p00*p40+p50*p00)-0.00450121*(p60*p00+p70*p00)-0.00532187*(p00*p80+p90*p00)-0.00438516*(p100*p00)+0.0042292*(p110*p00)+0.00704984*(p120*p00+p130*p00)-0.0025048*(p140*p00+p150*p00)-0.00684347*(p00*p160+p170*p00)-0.00600997*(p180*p00+p190*p00)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.00264485*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.0045403*(p240*p250*p00+p200*p00*p260+p230*p270*p00+p210*p00*p280)+0.00319095*(p00*p290*p200+p300*p00*p70+p00*p300*p210+p290*p00*p60)-0.00210824*(p00*p20*p310+p30*p00*p90+p310*p80*p00+p00*p30*p310+p20*p00*p80+p310*p90*p00)-0.00404378*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 11){
     ...
     energy = energy+0.0256033*(p00*p10)+0.0172747*(p10*p20+p30*p10)+0.00440368*(p40*p10+p10*p50)-0.00450121*(p60*p10+p70*p10)-0.00532187*(p80*p10+p10*p90)-0.00438516*(p100*p10)+0.0042292*(p110*p10)+0.00704984*(p120*p10+p130*p10)-0.0025048*(p140*p10+p150*p10)-0.00684347*(p160*p10+p10*p170)-0.00600997*(p180*p10+p190*p10)-0.00999758*(p200*p00*p10+p210*p00*p10)-0.00447681*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.00264485*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.0045403*(p240*p250*p10+p200*p10*p260+p230*p270*p10+p210*p10*p280)+0.00319095*(p290*p10*p70+p10*p300*p200+p300*p10*p60+p10*p290*p210)-0.00210824*(p310*p90*p10+p10*p20*p310+p30*p10*p80+p310*p80*p10+p10*p30*p310+p20*p10*p90)-0.00404378*(p220*p10*p320);
     return energy;
  }
...
double Monte_Carlo::normalized_pointenergy(int i, int j, int k, int b){
  ...
  if(b == 0){
     ...
     energy = energy+0.0705851*(p00)-0.0113244*(p00*p10+p20*p00)-0.0264636*(p00*p30+p40*p00+p00*p50+p60*p00)-0.0022506*(p00*p70+p00*p80+p00*p90+p00*p100)-0.0044953*(p110*p00+p120*p00)-0.00219258*(p00*p130+p00*p140)+0.0021146*(p00*p150+p00*p160)+0.00352492*(p00*p170+p00*p180+p00*p190+p00*p200)-0.0012524*(p00*p210+p00*p220+p00*p230+p00*p240)-0.00300498*(p00*p250+p00*p260+p00*p270+p00*p280)-0.00333253*(p00*p290*p300+p00*p310*p320)-0.000881616*(p00*p330*p340+p00*p350*p360+p00*p330*p370+p00*p350*p380)-0.00151343*(p00*p300*p370+p00*p290*p380+p00*p320*p340+p00*p310*p360)+0.00106365*(p290*p100*p00+p300*p90*p00+p310*p80*p00+p320*p70*p00)-0.00134793*(p120*p390*p00+p110*p400*p00+p00*p330*p110+p00*p350*p120);
     return energy;
  }
  ...
  if(b == 1){
     ...
     energy = energy+0.0705851*(p00)-0.0113244*(p00*p10+p20*p00)-0.0264636*(p00*p30+p40*p00+p00*p50+p60*p00)-0.0022506*(p00*p70+p00*p80+p00*p90+p00*p100)-0.0044953*(p00*p110+p00*p120)-0.00219258*(p00*p130+p00*p140)+0.0021146*(p00*p150+p00*p160)+0.00352492*(p00*p170+p00*p180+p00*p190+p00*p200)-0.0012524*(p00*p210+p00*p220+p00*p230+p00*p240)-0.00300498*(p00*p250+p00*p260+p00*p270+p00*p280)-0.00333253*(p00*p290*p300+p00*p310*p320)-0.000881616*(p00*p330*p340+p00*p350*p360+p00*p330*p370+p00*p350*p380)-0.00151343*(p00*p300*p370+p00*p290*p380+p00*p320*p340+p00*p310*p360)+0.00106365*(p290*p100*p00+p300*p90*p00+p310*p80*p00+p320*p70*p00)-0.00134793*(p00*p330*p120+p00*p350*p110+p110*p390*p00+p120*p400*p00);
     return energy;
  }
  ...
  if(b == 2){
     ...
     energy = energy+0.0705851*(p00)-0.0113244*(p00*p10+p20*p00)-0.0264636*(p30*p00+p00*p40+p50*p00+p00*p60)-0.0022506*(p00*p70+p00*p80+p00*p90+p00*p100)-0.0044953*(p110*p00+p120*p00)-0.00219258*(p00*p130+p00*p140)+0.0021146*(p00*p150+p00*p160)+0.00352492*(p00*p170+p00*p180+p00*p190+p00*p200)-0.0012524*(p00*p210+p00*p220+p00*p230+p00*p240)-0.00300498*(p00*p250+p00*p260+p00*p270+p00*p280)-0.00333253*(p00*p290*p300+p00*p310*p320)-0.000881616*(p00*p330*p340+p00*p350*p360+p00*p330*p370+p00*p350*p380)-0.00151343*(p00*p300*p370+p00*p290*p380+p00*p320*p340+p00*p310*p360)+0.00106365*(p290*p100*p00+p300*p90*p00+p310*p80*p00+p320*p70*p00)-0.00134793*(p120*p390*p00+p110*p400*p00+p00*p330*p110+p00*p350*p120);
     return energy;
  }
  ...
  if(b == 3){
     ...
     energy = energy+0.0705851*(p00)-0.0113244*(p00*p10+p20*p00)-0.0264636*(p30*p00+p00*p40+p50*p00+p00*p60)-0.0022506*(p00*p70+p00*p80+p00*p90+p00*p100)-0.0044953*(p00*p110+p00*p120)-0.00219258*(p00*p130+p00*p140)+0.0021146*(p00*p150+p00*p160)+0.00352492*(p00*p170+p00*p180+p00*p190+p00*p200)-0.0012524*(p00*p210+p00*p220+p00*p230+p00*p240)-0.00300498*(p00*p250+p00*p260+p00*p270+p00*p280)-0.00333253*(p00*p290*p300+p00*p310*p320)-0.000881616*(p00*p330*p340+p00*p350*p360+p00*p330*p370+p00*p350*p380)-0.00151343*(p00*p300*p370+p00*p290*p380+p00*p320*p340+p00*p310*p360)+0.00106365*(p290*p100*p00+p300*p90*p00+p310*p80*p00+p320*p70*p00)-0.00134793*(p00*p330*p120+p00*p350*p110+p110*p390*p00+p120*p400*p00);
     return energy;
  }
  ...
  if(b == 4){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p00*p20+p30*p00)+0.00220184*(p00*p40+p50*p00)-0.0022506*(p60*p00+p70*p00)-0.00266093*(p80*p00+p00*p90)-0.00219258*(p100*p00)+0.0021146*(p110*p00)+0.00352492*(p120*p00+p130*p00)-0.0012524*(p140*p00+p150*p00)-0.00342173*(p00*p160+p170*p00)-0.00300498*(p180*p00+p190*p00)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.000881616*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.00151343*(p200*p00*p250+p240*p260*p00+p210*p00*p270+p230*p280*p00)+0.00106365*(p290*p00*p70+p00*p300*p200+p300*p00*p60+p00*p290*p210)-0.000702748*(p310*p90*p00+p00*p20*p310+p30*p00*p80+p310*p80*p00+p00*p30*p310+p20*p00*p90)-0.00134793*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 5){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p10*p20+p30*p10)+0.00220184*(p40*p10+p10*p50)-0.0022506*(p60*p10+p70*p10)-0.00266093*(p10*p80+p90*p10)-0.00219258*(p100*p10)+0.0021146*(p110*p10)+0.00352492*(p120*p10+p130*p10)-0.0012524*(p140*p10+p150*p10)-0.00342173*(p160*p10+p10*p170)-0.00300498*(p180*p10+p190*p10)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.000881616*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.00151343*(p200*p10*p250+p240*p260*p10+p210*p10*p270+p230*p280*p10)+0.00106365*(p10*p290*p200+p300*p10*p70+p10*p300*p210+p290*p10*p60)-0.000702748*(p10*p20*p310+p30*p10*p90+p310*p80*p10+p10*p30*p310+p20*p10*p80+p310*p90*p10)-0.00134793*(p220*p10*p320);
     return energy;
  }
  ...
  if(b == 6){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p00*p20+p30*p00)+0.00220184*(p00*p40+p50*p00)-0.0022506*(p60*p00+p70*p00)-0.00266093*(p00*p80+p90*p00)-0.00219258*(p100*p00)+0.0021146*(p110*p00)+0.00352492*(p120*p00+p130*p00)-0.0012524*(p140*p00+p150*p00)-0.00342173*(p00*p160+p170*p00)-0.00300498*(p180*p00+p190*p00)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.000881616*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.00151343*(p200*p00*p250+p240*p260*p00+p210*p00*p270+p230*p280*p00)+0.00106365*(p00*p290*p200+p300*p00*p70+p00*p300*p210+p290*p00*p60)-0.000702748*(p00*p20*p310+p30*p00*p90+p310*p80*p00+p00*p30*p310+p20*p00*p80+p310*p90*p00)-0.00134793*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 7){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p10*p20+p30*p10)+0.00220184*(p40*p10+p10*p50)-0.0022506*(p60*p10+p70*p10)-0.00266093*(p80*p10+p10*p90)-0.00219258*(p100*p10)+0.0021146*(p110*p10)+0.00352492*(p120*p10+p130*p10)-0.0012524*(p140*p10+p150*p10)-0.00342173*(p160*p10+p10*p170)-0.00300498*(p180*p10+p190*p10)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.000881616*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.00151343*(p200*p10*p250+p240*p260*p10+p210*p10*p270+p230*p280*p10)+0.00106365*(p290*p10*p70+p10*p300*p200+p300*p10*p60+p10*p290*p210)-0.000702748*(p310*p90*p10+p10*p20*p310+p30*p10*p80+p310*p80*p10+p10*p30*p310+p20*p10*p90)-0.00134793*(p220*p10*p320);
     return energy;
  }
  ...
  if(b == 8){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p00*p20+p30*p00)+0.00220184*(p00*p40+p50*p00)-0.0022506*(p60*p00+p70*p00)-0.00266093*(p80*p00+p00*p90)-0.00219258*(p100*p00)+0.0021146*(p110*p00)+0.00352492*(p120*p00+p130*p00)-0.0012524*(p140*p00+p150*p00)-0.00342173*(p00*p160+p170*p00)-0.00300498*(p180*p00+p190*p00)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.000881616*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.00151343*(p240*p250*p00+p200*p00*p260+p230*p270*p00+p210*p00*p280)+0.00106365*(p290*p00*p70+p00*p300*p200+p300*p00*p60+p00*p290*p210)-0.000702748*(p310*p90*p00+p00*p20*p310+p30*p00*p80+p310*p80*p00+p00*p30*p310+p20*p00*p90)-0.00134793*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 9){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p10*p20+p30*p10)+0.00220184*(p40*p10+p10*p50)-0.0022506*(p60*p10+p70*p10)-0.00266093*(p10*p80+p90*p10)-0.00219258*(p100*p10)+0.0021146*(p110*p10)+0.00352492*(p120*p10+p130*p10)-0.0012524*(p140*p10+p150*p10)-0.00342173*(p160*p10+p10*p170)-0.00300498*(p180*p10+p190*p10)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.000881616*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.00151343*(p240*p250*p10+p200*p10*p260+p230*p270*p10+p210*p10*p280)+0.00106365*(p10*p290*p200+p300*p10*p70+p10*p300*p210+p290*p10*p60)-0.000702748*(p10*p20*p310+p30*p10*p90+p310*p80*p10+p10*p30*p310+p20*p10*p80+p310*p90*p10)-0.00134793*(p220*p10*p320);
     return energy;
  }
  ...
  if(b == 10){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p00*p20+p30*p00)+0.00220184*(p00*p40+p50*p00)-0.0022506*(p60*p00+p70*p00)-0.00266093*(p00*p80+p90*p00)-0.00219258*(p100*p00)+0.0021146*(p110*p00)+0.00352492*(p120*p00+p130*p00)-0.0012524*(p140*p00+p150*p00)-0.00342173*(p00*p160+p170*p00)-0.00300498*(p180*p00+p190*p00)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p50+p10*p00*p30+p40*p20*p00+p00*p10*p40+p10*p00*p20+p50*p30*p00)-0.000881616*(p220*p00*p30+p230*p20*p00+p220*p00*p20+p240*p30*p00)-0.00151343*(p240*p250*p00+p200*p00*p260+p230*p270*p00+p210*p00*p280)+0.00106365*(p00*p290*p200+p300*p00*p70+p00*p300*p210+p290*p00*p60)-0.000702748*(p00*p20*p310+p30*p00*p90+p310*p80*p00+p00*p30*p310+p20*p00*p80+p310*p90*p00)-0.00134793*(p220*p00*p320);
     return energy;
  }
  ...
  if(b == 11){
     ...
     energy = energy+0.0128017*(p00*p10)+0.00863735*(p10*p20+p30*p10)+0.00220184*(p40*p10+p10*p50)-0.0022506*(p60*p10+p70*p10)-0.00266093*(p80*p10+p10*p90)-0.00219258*(p100*p10)+0.0021146*(p110*p10)+0.00352492*(p120*p10+p130*p10)-0.0012524*(p140*p10+p150*p10)-0.00342173*(p160*p10+p10*p170)-0.00300498*(p180*p10+p190*p10)-0.00333253*(p200*p00*p10+p210*p00*p10)-0.00149227*(p00*p10*p30+p50*p20*p10+p10*p00*p40+p00*p10*p20+p40*p30*p10+p10*p00*p50)-0.000881616*(p220*p10*p30+p230*p20*p10+p220*p10*p20+p240*p30*p10)-0.00151343*(p240*p250*p10+p200*p10*p260+p230*p270*p10+p210*p10*p280)+0.00106365*(p290*p10*p70+p10*p300*p200+p300*p10*p60+p10*p290*p210)-0.000702748*(p310*p90*p10+p10*p20*p310+p30*p10*p80+p310*p80*p10+p10*p30*p310+p20*p10*p90)-0.00134793*(p220*p10*p320);
     return energy;
  }
...
void Monte_Carlo::pointcorr(int i, int j, int k, int b){
  ...
  if(b == 0){
     ...
     AVcorr[0]+=1.0/12;
     AVcorr[1]+=(p0)/4;
     AVcorr[8]+=(p0*p1+p2*p0)/8;
     AVcorr[14]+=(p0*p3+p4*p0+p0*p5+p6*p0)/16;
     AVcorr[15]+=(p0*p7+p0*p8+p0*p9+p0*p10)/32;
     AVcorr[19]+=(p11*p0+p12*p0)/8;
     AVcorr[27]+=(p0*p13+p0*p14)/16;
     AVcorr[29]+=(p0*p15+p0*p16)/16;
     AVcorr[38]+=(p0*p17+p0*p18+p0*p19+p0*p20)/32;
     AVcorr[39]+=(p0*p21+p0*p22+p0*p23+p0*p24)/32;
     AVcorr[46]+=(p0*p25+p0*p26+p0*p27+p0*p28)/32;
     AVcorr[115]+=(p0*p29*p30+p0*p31*p32)/24;
     AVcorr[125]+=(p0*p33*p34+p0*p35*p36+p0*p33*p37+p0*p35*p38)/48;
     AVcorr[131]+=(p0*p30*p37+p0*p29*p38+p0*p32*p34+p0*p31*p36)/48;
     AVcorr[137]+=(p29*p10*p0+p30*p9*p0+p31*p8*p0+p32*p7*p0)/48;
     AVcorr[161]+=(p12*p39*p0+p11*p40*p0+p0*p33*p11+p0*p35*p12)/24;
     return;
  }
  ...
  if(b == 1){
     ...
     AVcorr[131]+=(p0*p30*p37+p0*p29*p38+p0*p32*p34+p0*p31*p36)/48;
     AVcorr[137]+=(p29*p10*p0+p30*p9*p0+p31*p8*p0+p32*p7*p0)/48;
     AVcorr[161]+=(p0*p33*p12+p0*p35*p11+p11*p39*p0+p12*p40*p0)/24;
     return;
  }
  ...
  if(b == 2){
     ...
     AVcorr[1]+=(p0)/4;
     AVcorr[8]+=(p0*p1+p2*p0)/8;
     AVcorr[14]+=(p3*p0+p0*p4+p5*p0+p0*p6)/16;
     AVcorr[15]+=(p0*p7+p0*p8+p0*p9+p0*p10)/32;
     AVcorr[19]+=(p11*p0+p12*p0)/8;
     AVcorr[27]+=(p0*p13+p0*p14)/16;
     AVcorr[29]+=(p0*p15+p0*p16)/16;
  ...
  if(b == 4){
     ...
     AVcorr[0]+=1.0/12;
     AVcorr[5]+=(p0*p1)/8;
     AVcorr[7]+=(p0*p2+p3*p0)/16;
     AVcorr[11]+=(p0*p4+p5*p0)/16;
     AVcorr[15]+=(p6*p0+p7*p0)/32;
     AVcorr[16]+=(p8*p0+p0*p9)/16;
     AVcorr[27]+=(p10*p0)/16;
     AVcorr[29]+=(p11*p0)/16;
     AVcorr[38]+=(p12*p0+p13*p0)/32;
     AVcorr[39]+=(p14*p0+p15*p0)/32;
     AVcorr[41]+=(p0*p16+p17*p0)/16;
     AVcorr[46]+=(p18*p0+p19*p0)/32;
     AVcorr[115]+=(p20*p0*p1+p21*p0*p1)/24;
     AVcorr[123]+=(p0*p1*p5+p1*p0*p3+p4*p2*p0+p0*p1*p4+p1*p0*p2+p5*p3*p0)/48;
     AVcorr[125]+=(p22*p0*p3+p23*p2*p0+p22*p0*p2+p24*p3*p0)/48;
     AVcorr[131]+=(p20*p0*p25+p24*p26*p0+p21*p0*p27+p23*p28*p0)/48;
     AVcorr[137]+=(p29*p0*p7+p0*p30*p20+p30*p0*p6+p0*p29*p21)/48;
     AVcorr[144]+=(p31*p9*p0+p0*p2*p31+p3*p0*p8+p31*p8*p0+p0*p3*p31+p2*p0*p9)/48;
     AVcorr[161]+=(p22*p0*p32)/24;
     return;
  }
  ...
  if(b == 5){
     ...
     AVcorr[0]+=1.0/12;
     AVcorr[5]+=(p0*p1)/8;
     AVcorr[7]+=(p1*p2+p3*p1)/16;
     AVcorr[11]+=(p4*p1+p1*p5)/16;
     AVcorr[15]+=(p6*p1+p7*p1)/32;
     AVcorr[16]+=(p1*p8+p9*p1)/16;
     AVcorr[27]+=(p10*p1)/16;
     AVcorr[29]+=(p11*p1)/16;
     AVcorr[38]+=(p12*p1+p13*p1)/32;
     AVcorr[39]+=(p14*p1+p15*p1)/32;
     AVcorr[41]+=(p16*p1+p1*p17)/16;
     AVcorr[46]+=(p18*p1+p19*p1)/32;
     AVcorr[115]+=(p20*p0*p1+p21*p0*p1)/24;
     AVcorr[123]+=(p0*p1*p3+p5*p2*p1+p1*p0*p4+p0*p1*p2+p4*p3*p1+p1*p0*p5)/48;
     AVcorr[125]+=(p22*p1*p3+p23*p2*p1+p22*p1*p2+p24*p3*p1)/48;
     AVcorr[131]+=(p20*p1*p25+p24*p26*p1+p21*p1*p27+p23*p28*p1)/48;
     AVcorr[137]+=(p1*p29*p20+p30*p1*p7+p1*p30*p21+p29*p1*p6)/48;
     AVcorr[144]+=(p1*p2*p31+p3*p1*p9+p31*p8*p1+p1*p3*p31+p2*p1*p8+p31*p9*p1)/48;
     AVcorr[161]+=(p22*p1*p32)/24;
     return;
  }
  ...
  if(b == 6){
     ...
     AVcorr[125]+=(p22*p0*p3+p23*p2*p0+p22*p0*p2+p24*p3*p0)/48;
     AVcorr[131]+=(p20*p0*p25+p24*p26*p0+p21*p0*p27+p23*p28*p0)/48;
     AVcorr[137]+=(p0*p29*p20+p30*p0*p7+p0*p30*p21+p29*p0*p6)/48;
     AVcorr[144]+=(p0*p2*p31+p3*p0*p9+p31*p8*p0+p0*p3*p31+p2*p0*p8+p31*p9*p0)/48;
     AVcorr[161]+=(p22*p0*p32)/24;
     return;
  }
  ...
  if(b == 7){
     ...
     AVcorr[125]+=(p22*p1*p3+p23*p2*p1+p22*p1*p2+p24*p3*p1)/48;
     AVcorr[131]+=(p20*p1*p25+p24*p26*p1+p21*p1*p27+p23*p28*p1)/48;
     AVcorr[137]+=(p29*p1*p7+p1*p30*p20+p30*p1*p6+p1*p29*p21)/48;
     AVcorr[144]+=(p31*p9*p1+p1*p2*p31+p3*p1*p8+p31*p8*p1+p1*p3*p31+p2*p1*p9)/48;
     AVcorr[161]+=(p22*p1*p32)/24;
     return;
  }
  ...
  if(b == 8){
     ...
     AVcorr[123]+=(p0*p1*p5+p1*p0*p3+p4*p2*p0+p0*p1*p4+p1*p0*p2+p5*p3*p0)/48;
     AVcorr[125]+=(p22*p0*p3+p23*p2*p0+p22*p0*p2+p24*p3*p0)/48;
     AVcorr[131]+=(p24*p25*p0+p20*p0*p26+p23*p27*p0+p21*p0*p28)/48;
     AVcorr[137]+=(p29*p0*p7+p0*p30*p20+p30*p0*p6+p0*p29*p21)/48;
     AVcorr[144]+=(p31*p9*p0+p0*p2*p31+p3*p0*p8+p31*p8*p0+p0*p3*p31+p2*p0*p9)/48;
     AVcorr[161]+=(p22*p0*p32)/24;
     return;
  ...
  if(b == 9){
     ...
     AVcorr[123]+=(p0*p1*p3+p5*p2*p1+p1*p0*p4+p0*p1*p2+p4*p3*p1+p1*p0*p5)/48;
     AVcorr[125]+=(p22*p1*p3+p23*p2*p1+p22*p1*p2+p24*p3*p1)/48;
     AVcorr[131]+=(p24*p25*p1+p20*p1*p26+p23*p27*p1+p21*p1*p28)/48;
     AVcorr[137]+=(p1*p29*p20+p30*p1*p7+p1*p30*p21+p29*p1*p6)/48;
     AVcorr[144]+=(p1*p2*p31+p3*p1*p9+p31*p8*p1+p1*p3*p31+p2*p1*p8+p31*p9*p1)/48;
     AVcorr[161]+=(p22*p1*p32)/24;
     return;
